...
const ReportCase = () => {
  ...
  const handlePhotoChange = async (e) => {
    ...
    if (location) {
      const payload = {
        bucket_name: "crime-management-cloud",
        object_key: location,
      };
      console.log("Payload", payload);
      const response = await axios.post(
        "https://v3pcbfrlfa.execute-api.us-east-2.amazonaws.com/v3/compare-face",
        payload
      );
      ...
      if (response.data.success == true) {
        ...
        const resp = await axios.get("http://localhost:7000/criminal/objkey", {
        ...
        setCriminalId(criminalDetails._id);
  ...
  const handleFirChange = async (e) => {
    ...
    const response = await axios.post(
      "https://v3pcbfrlfa.execute-api.us-east-2.amazonaws.com/v2/extract-text",
      payload
    );
  ...
  const handleSubmit = async (e) => {
    ...
    if (!imageMatched) {
      ...
      const response = await axios.post("http://localhost:7000/criminal/create", criminalPayload);
      console.log(response.data);
      console.log("Criminal ID", response.data.data._id);
  
      await setCriminalId(response.data.data._id);

      reportPayload = {
        description: description,
        type: type,
        date: date,
        criminalId: response.data.data._id,
        firKey: firKey
      };
      ...
      const reportResponse = await axios.post("http://localhost:7000/report/create", reportPayload);
